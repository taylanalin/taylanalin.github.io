const book = {
  ...
  kicker: "Saha Hikâyeleriyle Teknoloji Okuma Atlası",
  subtitle: "Bilgisayarın iç düzeninden bulut, güvenlik, yapay zekâ ve teknik liderliğe uzanan yolu; ürün ezberiyle değil, gerçek kurum sahneleri ve karar diliyle anlatır.",
  promise:
    "Bu atlas tek tek araçları parlatmak yerine onların arkasındaki düşünme biçimini gösterir: bir sorun nasıl belirti verir, ekipler nasıl konuşur, karar hangi kanıtla alınır ve teknoloji kurum hayatında nasıl gerçek sonuç üretir."
};
...
const categories = [
  {
    ...
    summary: "Donanım, işletim sistemi, terminal, sunucu pratiği, ağ mantığı, web protokolleri ve versiyon kontrolüyle teknik zemini kurar.",
    mode: "systems",
    chapters: [
      ...
      "Sunucu işletim sistemi pratiği: Linux kullanıcıları, izinler, servisler ve paket yönetimi",
      "Ağ temelleri: TCP/IP, port, subnet, gateway ve routing",
      "Web protokolleri: DNS, HTTP ve HTTPS ile isteğin görünmeyen yolculuğu",
      "Versiyon kontrolü ve ekip hafızası: Git, GitHub, branch ve pull request",
  ...
  },
  {
    ...
    chapters: [
      ...
      "Veri yapıları ve algoritma sezgisi: liste, sözlük, yığın, kuyruk ve Big O",
      "Fonksiyonlar, modüller ve okunabilir kod alışkanlığı",
      "Nesne yönelimli düşünme: sınıf, nesne, kapsülleme ve tasarım sezgisi",
      "Hata yönetimi ve veri formatları: dosya, JSON, doğrulama ve kötü gün senaryoları",
      "Sistemlerle konuşmak: API isteği, cevap okuma ve hata ayıklama",
      "İlişkisel veri düşüncesi: SQL, tablo, JOIN, index ve sorgu mantığı",
      "Kalıcı kayıt ve veri bütünlüğü: PostgreSQL, transaction ve constraint",
      ...
      "Üretime hazır küçük proje: kapsam, test, paketleme ve anlatılabilir portföy"
    ]
  },
  {
    id: "asama-3-web-api",
    ...
    chapters: [
      ...
      "Önbellek ve içerik dağıtımı: Redis, cache ve CDN ile hız-tutarlılık dengesi",
      "Asenkron çalışma ve mesaj kuyrukları: Kafka, RabbitMQ ve olay tabanlı akış",
  ...
  },
  {
    ...
    chapters: [
      ...
      "Araç kullanımı ve function calling: modelin sistemlerle güvenli konuşması",
      "Yerel model çalıştırma: Ollama, mahremiyet, kapasite ve kalite dengesi",
      "Ajan bağlamı ve araç protokolü: MCP ile veriyi ve yetenekleri standart bağlamak",
  ...
  },
  {
    ...
    summary: "Üretim yaşam döngüsü, konteyner, orkestrasyon, CI/CD, monitoring, bulut sağlayıcıları, yedekleme, maliyet ve işletim disiplini üzerine kurulur.",
    mode: "systems",
    chapters: [
      ...
      "Bulut sağlayıcıları ve yönetilen servisler: AWS, Google Cloud, compute, storage ve network",
  ...
  },
  {
    ...
    chapters: [
      ...
      "Kimlik ve erişim güvenliği: IAM, MFA, SSO, PAM ve least privilege",
      "Ağ ve uygulama sınırı güvenliği: firewall, IDS/IPS, WAF, segmentasyon ve Zero Trust",
      ...
      "Güvenlik izleme ve SOC pratiği: log, SIEM ve alarmdan olaya giden karar hattı",
  ...
  },
  {
    ...
    chapters: [
      ...
      "E-ticaret sipariş yolculuğu: katalogdan sepete, ödemeden teslimata",
      "Ödeme sisteminde saniyelerin hesabı",
      "Kargo ve lojistik takip mimarisi",
      "Kamu portalı başvuru akışı",
      ...
      "Müşteri temas merkezi ve CRM entegrasyonu",
  ...
  },
  {
    ...
    chapters: [
      ...
      "Web servisleri, kimlik, cache ve asenkron akış soruları",
      "Sistem işletimi, konteyner, orkestrasyon ve bulut soruları",
...
const articleSupplements = {
  "buyuk-mimariler-9": `Komuta kontrol ve simülasyon tarafında işin kalbi gerçek zamanlı durumsal farkındalıktır. Harita, sensör, araç, personel, görev emri, alarm, telsiz kaydı ve simülasyon senaryosu aynı ekranda buluştuğunda mimari yalnız veri taşımaz; kararın gecikmesini azaltır. Bu yüzden GIS katmanı, message broker, event store, düşük gecikmeli ağ, rol bazlı yetki, audit log ve replay edilebilir senaryo kayıtları birlikte düşünülür. Simülasyonun değeri de burada çıkar: canlı sistemi riske atmadan trafik artışı, bağlantı kopması, yanlış emir, geciken telemetri veya koordinasyon hatası prova edilebilir. Böyle bir mimaride en pahalı hata, güzel ekranın arkasında güvenilir zaman damgası, sahiplik ve iz kaydı olmamasıdır.`,
  "buyuk-mimariler-10": `Çağrı merkezi ve CRM entegrasyonunda kullanıcı ekranda tek müşteri kartı görür; arkada IVR, CTI, santral, ticket sistemi, kampanya aracı, kimlik doğrulama, KVKK onayı ve işlem geçmişi birlikte çalışır. Temsilci telefonu açtığında müşteri bilgisi ekrana düşmüyorsa saniyeler uzar, müşteri tekrar tekrar derdini anlatır, kurumun hafızası parçalanır. Bu yüzden screen pop, omnichannel geçmiş, çağrı kaydı, yetki sınırı, maskeleme, not şablonu ve SLA takibi önemli hale gelir. İyi entegrasyon temsilciyi hızlandırırken mahremiyeti de korur; herkes her veriyi görmez, her işlem iz bırakır, müşteri hikâyesi departmanlar arasında kaybolmadan akar.`,
  "buyuk-mimariler-13": `Raporlama ve veri ambarı akışında asıl mesele grafik güzelliği değil, sayının güvenilirliğidir. ETL ya da ELT hattı kaynak sistemlerden veriyi alır; müşteri, işlem, ürün, bölge ve zaman tanımlarını ortaklaştırır; data warehouse içinde yıldız şema, fact ve dimension tablolarıyla karar verilebilir bir zemin kurar. Semantic layer yoksa aynı gösterge farklı ekiplerde farklı hesaplanır. Data lineage yoksa bir rakamın nereden geldiği kavga konusu olur. Kalite kuralları, gecikme alarmı, yetki modeli, kişisel veri maskeleme ve rapor sahipliği bu yüzden teknik ayrıntı değil, yönetim güveninin altyapısıdır.`,
  "buyuk-mimariler-14": `Mobil uygulama ile backend arasındaki yol masaüstü webden daha kırılgandır; çünkü ağ değişir, cihaz uykuya geçer, uygulama eski sürümde kalır, kullanıcı metroda işlem yapmaya çalışır. Bu yüzden API versioning, access token yenileme, refresh token güvenliği, rate limit, push notification token, offline cache, retry politikası ve cihaz telemetrisi birlikte tasarlanır. Backend hatayı açık söylemeli, mobil uygulama kullanıcıyı çıkmazda bırakmamalıdır. Bir ödeme yarıda kaldığında uygulama tekrar basınca çift işlem üretmemeli; idempotency burada yalnız mimari terim değil, kullanıcının parasını ve kurumun itibarını koruyan küçük kilittir.`,
  "proje-kamu-yonetim-1": `Kurumsal projenin ilk tuzağı takvimi hedef sanmaktır. Oysa takvim yalnız ritim verir; projenin değeri hangi vatandaş hizmetini hızlandırdığı, hangi operasyon yükünü azalttığı, hangi hatayı görünür kıldığı veya hangi kararı daha güvenilir hale getirdiğiyle ölçülür. Bu yüzden başlangıçta fayda hipotezi, başarı ölçütü, paydaş beklentisi ve kapsam dışı bırakılan işler açık yazılmalıdır. Bir proje "portal yapılacak" diye değil, "başvuru süresi şu kadar kısalacak, veri şu kaynaktan doğrulanacak, kullanıcı şu adımı tekrar etmeyecek" diye güçlenir. Değer cümlesi net değilse en düzgün Gantt şeması bile güzel çizilmiş bir belirsizliktir.`,
  "proje-kamu-yonetim-2": `İhtiyaç analizi, kullanıcının söylediği ilk isteği doğrudan şartnameye yazmak değildir. Kullanıcı çoğu zaman çözüm sandığı şeyi ister; iyi analist önce as-is akışı dinler, darboğazı bulur, sonra to-be akışı kurar. Kullanıcı hikâyesi, iş kuralı, veri sahibi, istisna senaryosu, entegrasyon ihtiyacı ve kabul kriteri aynı masada netleşmelidir. "Rapor istiyoruz" cümlesi bazen veri kalitesi sorununu, bazen yetki karmaşasını, bazen de yöneticinin günlük görünürlük ihtiyacını saklar. Gereksinim doğru ayrıştırılırsa kapsam yönetilebilir olur; ayrıştırılmazsa proje ilerledikçe herkes başka bir sistemi hayal ettiğini fark eder.`,
  "proje-kamu-yonetim-3": `Teknik şartname iyi yazıldığında tedarikçiyi boğmaz, kabul gününü rahatlatır. Ölçülebilir olmayan madde anlaşmazlık üretir: "hızlı çalışacaktır" yerine hedef cevap süresi, eşzamanlı kullanıcı, log tutma süresi, yedekleme sıklığı, erişilebilirlik oranı, güvenlik testi ve entegrasyon formatı yazılmalıdır. Kabul de yalnız ekrana bakmak değildir; test senaryosu, örnek veri, demo kaydı, kurulum dokümanı, kaynak kod teslimi, lisans bilgisi, eğitim tutanağı ve hata listesiyle kanıtlanır. İyi şartname gelecekteki tartışmayı bugünden azaltır; taraflara neyin başarı, neyin eksik, neyin değişiklik talebi olduğunu sakin biçimde söyler.`,
  "proje-kamu-yonetim-4": `Paydaş haritası çizilmezse proje görünmez güçler tarafından yönetilir. Talep sahibi faydayı ister, bilgi işlem sürdürülebilirliği düşünür, güvenlik sınırı çizer, hukuk kişisel veriye bakar, satın alma sözleşmeyi korur, kullanıcı günlük işinin bozulmamasını ister. RACI bu kalabalığı disipline eder: kim sorumlu, kim hesap verebilir, kime danışılır, kim bilgilendirilir? Karar makamı belirsizse toplantılar çoğalır ama karar doğmaz. Özellikle kamu projelerinde imza yetkisi, veri sahipliği, bütçe onayı ve kabul komisyonu baştan görünür olmalıdır; yoksa teknik ilerleme idari boşluğa çarpar.`,
  "proje-kamu-yonetim-5": `Risk, varsayım, sorun kaydı ve kapsam kayması aynı defterde ama farklı anlamlarda yaşar. Risk henüz olmamış ihtimaldir; varsayım doğru kabul edilen belirsizliktir; sorun gerçekleşmiş etkidir; kapsam kayması ise projenin sessizce büyümesidir. Bunları karıştıran ekip her toplantıda aynı şeyi yeniden tartışır. İyi kayıt; olasılık, etki, sahip, aksiyon, son tarih ve karar alanlarını taşır. "Tedarikçi API dokümanını zamanında verecek" bir varsayımdır; gecikirse risk gerçekleşir; iş planı etkilenirse sorun olur; bu yüzden ekstra ekran istenirse kapsam değişir. Yönetim disiplini bu ayrımı sakin biçimde korumaktır.`,
  "proje-kamu-yonetim-7": `Tedarikçi yönetiminde SLA yalnız ceza maddesi değildir; hizmetin nasıl yaşatılacağını anlatan ortak nabızdır. Yanıt süresi, çözüm süresi, bakım penceresi, eskalasyon yolu, yedek ekip, lisans yenileme, güvenlik yaması, veri çıkış formatı ve sözleşme bitiş planı baştan konuşulmalıdır. İyi müşteri her şeyi tedarikçiye bırakmaz; iyi tedarikçi de her belirsizliği "kapsam dışı" diye itmez. Aylık hizmet raporu, açık hata listesi, değişiklik kaydı ve performans metriği ilişkiyi kişisel telefon trafiğinden çıkarıp yönetilebilir hale getirir. Tedarikçi bağımlılığı ancak ölçüm, kayıt ve çıkış planıyla sağlıklı kalır.`,
  "proje-kamu-yonetim-9": `Kamu BT işleyişinde merkez, taşra, belediye, üniversite ve savunma şirketi aynı kelimeleri kullansa da ritimleri farklıdır. Merkez politika ve standart üretir, taşra hizmeti gerçek kullanıcıyla yaşar, belediye vatandaş temasını doğrudan hisseder, üniversite akademik esneklikle idari düzeni birlikte taşır, savunma şirketi güvenlik ve teslim disipliniyle hareket eder. Bu yüzden tek tip çözüm çoğu zaman sahada zorlanır. Yetki devri, veri paylaşım protokolü, ağ erişimi, bakım saati, kullanıcı eğitimi ve destek modeli kuruma göre düşünülmelidir. İyi kamu BT dili mevzuatı, sahayı ve teknolojiyi aynı cümlede buluşturur.`,
  "proje-kamu-yonetim-11": `Proje ve kamu mülakatlarında güçlü aday mevzuat maddesi ezberleyen değil, sürecin neden var olduğunu anlayan kişidir. İhtiyaç yazısı, teknik şartname, yaklaşık maliyet, ihale, muayene kabul, KVKK değerlendirmesi, veri paylaşım protokolü ve bakım sözleşmesi birer bürokratik engel gibi değil, kurumun hesap verebilirlik mekanizması gibi anlatılmalıdır. Görüşmeci "bu işi nasıl yönetirdin?" dediğinde cevap küçük bir yolculuk kurmalıdır: ihtiyacı netleştiririm, paydaşları ayırırım, ölçülebilir kabul yazarım, riskleri kayda alırım, teslimi kanıtla kapatırım. Böyle anlatan aday süreci yaşamış gibi görünür çünkü olayın yalnız formunu değil, mantığını kavramıştır.`,
  "mulakat-ve-ust-duzey-hazirlik-1": `Mülakat stratejisinde en büyük fark, cevabı ezber cümle yerine düşünce zinciriyle kurmaktır. Görüşmeci "Redis nedir?" diye sorduğunda tek tanım yetmez; hangi problemi çözdüğünü, hangi veriyi cache etmeyeceğini, eski veri riskini nasıl yöneteceğini ve hangi metriğe bakacağını duymak ister. Aynı mantık Docker, SQL, Kubernetes, RAG veya IAM için de geçerlidir. Güçlü aday önce sade tanım verir, sonra örnek kurar, sonra trade-off söyler, sonra ölçüm ekler. Bilmediği yerde de dürüst davranır: "Bu ayrıntıyı ezbere söylemeyeyim, üretimde şu loga ve şu dokümana bakarım" cümlesi bazen sahici güven verir.`,
  "mulakat-ve-ust-duzey-hazirlik-8": `Sistem tasarımı mülakatında tahta, adayın zihninin sahnesidir. İlk çizgi kullanıcıdan başlar; sonra API gateway, kimlik doğrulama, servisler, veritabanı, cache, kuyruk, dosya depolama, arama, loglama ve izleme adım adım gelir. İyi aday her kutuyu çizmez; önce gereksinimi sorar, trafik varsayımı kurar, veri modelini sadeleştirir, darboğazı tahmin eder, sonra ölçekleme ve hata telafisini konuşur. "Milyon kullanıcı" cümlesi tek başına mimari kurdurmaz; okuma-yazma oranı, tutarlılık beklentisi, latency hedefi, veri kaybı toleransı ve ekip kapasitesi tasarımı belirler. Tahtadaki düzen, cevabın düzenidir.`,
  "mulakat-ve-ust-duzey-hazirlik-9": `Yöneticilik mülakatında teknik bilgi hâlâ önemlidir ama tek başına yetmez. Aday bütçe konuşurken toplam sahip olma maliyetini, risk konuşurken etki ve olasılığı, insan konuşurken ekip yükünü, karar konuşurken kanıt ve geri dönüş yolunu anlatmalıdır. "Bu sistemi buluta taşırım" demek yerine hangi verinin taşınacağını, hangi regülasyonun etkileneceğini, maliyetin nasıl izleneceğini, çıkış planının ne olacağını ve ekibin hangi yetkinliğe ihtiyaç duyacağını söylemek gerekir. Üst düzey görüşmede olgunluk, tek doğru cevabı bulmak değil; seçenekleri, bedelleri ve öneriyi sakin biçimde masaya koymaktır.`,
  "mulakat-ve-ust-duzey-hazirlik-10": `Kişisel gelişim rotası teknisyenlikten liderliğe düz bir merdiven değildir; bazen derinleşme, bazen genişleme ister. Bir dönem Linux, ağ ve veritabanı gibi zemini sağlamlaştırırsın; sonra yazılım mimarisi, bulut, güvenlik, veri ve yapay zekâ gibi komşu alanlarla konuşmayı öğrenirsin. Liderlik aşamasında ise bilgi kadar öğrenme sistemi önem kazanır: postmortem okumak, mimari karar kaydı incelemek, bütçe savunması yazmak, tedarikçi sözleşmesi çözmek, genç ekibe mentorluk etmek. Her şeyi bilmek imkânsızdır; ama hangi konuda kimi dinleyeceğini, hangi kanıtı isteyeceğini ve hangi riski görünür kılacağını öğrenmek mümkündür.`,
...
const articleContentAliases = {
  "asama-2-programlama-3": "asama-2-programlama-11",
  "asama-2-programlama-4": "asama-2-programlama-3",
  "asama-2-programlama-5": "asama-2-programlama-4",
  "asama-2-programlama-6": "asama-2-programlama-5",
  "asama-2-programlama-7": "asama-2-programlama-6",
  "asama-2-programlama-8": "asama-2-programlama-7",
  "asama-2-programlama-9": "asama-2-programlama-8",
  "asama-2-programlama-11": "asama-2-programlama-10",
  "asama-4-ai-temelleri-2": "asama-4-ai-temelleri-4",
  "asama-4-ai-temelleri-3": "asama-4-ai-temelleri-2",
  "asama-4-ai-temelleri-4": "asama-4-ai-temelleri-3",
  "asama-5-llm-gelistirme-4": "asama-5-llm-gelistirme-7",
  "asama-5-llm-gelistirme-5": "asama-5-llm-gelistirme-4",
  "asama-5-llm-gelistirme-6": "asama-5-llm-gelistirme-5",
  "asama-5-llm-gelistirme-7": "asama-5-llm-gelistirme-6"
};
...
const articleOverrides = {
  ...
  "asama-2-programlama-12": [
    "Üretime hazır küçük proje, portföydeki güzel vitrinden bir adım daha ileri gider. Burada amaç yalnız 'çalışıyor' demek değildir; başkasının projeyi açıp anlayabilmesi, kurabilmesi, test edebilmesi, hatayı izleyebilmesi ve kararlarını okuyabilmesidir. Küçük bir görev takip uygulaması bile bu ciddiyetle ele alınabilir: kapsam kısa yazılır, veri modeli sade çizilir, API sözleşmesi belirtilir, veritabanı migration dosyaları tutulur, hata cevapları rastgele bırakılmaz, README gerçekten çalıştırma yolu gösterir. Docker varsa yalnız havalı görünmek için değil, ortam farkını azaltmak için vardır. Testler her satırı kapsamak zorunda değildir ama kritik iş kuralını ve birkaç kötü gün senaryosunu korur. `.env` şablonu, örnek veri, loglama ve temel güvenlik notu da projeyi oyuncaktan ürüne yaklaştırır.",
    "Böyle bir proje mülakatta çok güçlü konuşur çünkü adaya yalnız kod yazdırmaz, karar verdirtir. Neden bu veritabanı, neden bu API yapısı, neden bu hata formatı, neden bu klasör düzeni, neden bu test? Aday bu sorulara sakin cevap veriyorsa proje küçük olsa bile olgun görünür. Üretime hazır düşünmek, her şeyi devasa yapmak değildir; küçük işi bile geri alınabilir, izlenebilir ve anlatılabilir yapmak demektir. Bir yönetici gözüyle bakıldığında da değer buradadır: proje bir kişinin bilgisayarında çalışan gizli bir deneme olmaktan çıkar, ekip hafızasına girebilecek düzenli bir parçaya dönüşür."
  ],
  "asama-5-llm-gelistirme-2": [
  ...
  "mulakat-ve-ust-duzey-hazirlik-4": [
    "Web servisleri, kimlik, cache ve asenkron akış sorularında görüşmeci aslında tek tek teknoloji adı değil, uçtan uca sistem sezgisi arar. Bir kullanıcı giriş yapar, yetkisi doğrulanır, API isteği backend'e gider, sık okunan veri cache'ten dönebilir, kritik kayıt veritabanında kalıcı hale gelir, uzun süren işler kuyruğa bırakılır, hata olduğunda log ve metrik iz üretir. Bu akışı kurmadan JWT, Redis ya da Kafka anlatmak kelime saymak olur. İyi aday önce web isteğinin yolunu çizer: tarayıcı, HTTP, kimlik, servis, veri, cevap ve izleme. Sonra her kavramı bu yolun bir noktasına yerleştirir.",
    "Bu başlıkta güçlü cevap, hız ile güvenilirlik arasındaki pazarlığı açık söyler. Cache hız kazandırır ama eski veri riski doğurur; token kullanıcı deneyimini kolaylaştırır ama süre, saklama yeri ve iptal stratejisi ister; mesaj kuyruğu ana isteği rahatlatır ama retry, dead-letter queue ve idempotency düşünülmezse hatayı geciktirir. Mülakatta bu alanı anlatırken tek bir sipariş ya da başvuru akışı seç: kullanıcı işlem yapar, kimlik doğrulanır, okuma cache'ten hızlanır, resmi kayıt veritabanına yazılır, bildirim kuyruğa bırakılır, hata durumunda anlamlı durum kodu ve log üretilir. Böyle cevap veren aday web'i ekran değil, yaşayan hizmet olarak gördüğünü hissettirir."
  ],
  "mulakat-ve-ust-duzey-hazirlik-5": [
    "Sistem işletimi, konteyner, orkestrasyon ve bulut soruları üretim ortamı sezgisini ölçer. Görüşmeci yalnız komut bilip bilmediğini değil, çalışan bir servisin nerede nefes aldığını, nerede tökezlediğini ve hangi kanıtla ayağa kaldırılacağını dinler. Bir servis cevap vermiyorsa önce süreç mi durmuş, port mu kapalı, DNS mi yanlış, sertifika mı bitmiş, logda hata mı var, kaynak mı tükenmiş diye sırayla bakabilmek gerekir. Konteyner tarafında image ile çalışan container ayrımı, volume ile kalıcı veri, environment variable ile konfigürasyon, registry ile dağıtım ilişkisi net olmalıdır. Orkestrasyonda ise pod'un neden restart ettiği, deployment'ın nasıl güncellendiği, service'in trafiği nasıl taşıdığı, ingress'in dış kapıyı nasıl açtığı anlatılmalıdır.",
    "Bulut soruları bu resmi kapasite, maliyet ve sorumluluk paylaşımıyla genişletir. IaaS, PaaS ve SaaS arasındaki fark yalnız hizmet adı değildir; hangi katmanı sen işletiyorsun, hangisini sağlayıcı yönetiyor, hangi log sende, hangi güvenlik ayarı senin sorumluluğunda? Güçlü aday Docker ya da Kubernetes adını öne atıp durmaz; önce çalışma modelini anlatır, sonra aracı yerine koyar. Örneğin küçük bir web servisini imaj haline getiririm, CI/CD hattında test ederim, registry'ye gönderirim, Kubernetes üzerinde rollout yaparım, health check ve resource limit tanımlarım, log ve metrikle izlerim, gerekirse rollback planını hazır tutarım. Bu cevap, üretim ortamını yalnız kurulum değil, sürekli yaşayan bir sorumluluk olarak gördüğünü gösterir."
  ]
};
...
function editorialDepthForShortArticle(category, chapter, text) {
  const baseWords = articleWordCount(text);
  if (baseWords >= 700) return "";
  const title = cleanTitle(chapter);
  const titleForSentence = title.charAt(0).toLocaleUpperCase("tr-TR") + title.slice(1);
  ...
  const sceneByMode = {
    systems: `${topic} başlığına sahadan bakınca ekranda tek bir kavram değil, çalışan bir düzen görünür. Bir tarafta CPU, RAM, disk, port, DNS, servis ve log gibi teknik işaretler; diğer tarafta bekleyen kullanıcı, duran rapor ve cevap isteyen yönetici vardır. Bu iki taraf aynı olayın parçaları olarak görülmediğinde bilgisayar bilgisi parça listesine, görüldüğünde ise işleyen bir haritaya dönüşür.`,
    software: `${topic} en iyi bir kod editörünün sessizliğinde değil, canlıya gidecek küçük bir değişikliğin çevresinde anlaşılır. Değişken, fonksiyon, sınıf, JSON, API, SQL veya test dediğimiz şeyler yalnız sözdizimi değildir; yanlış kurulduğunda kullanıcının gördüğü hata, loglarda beliren iz ve veritabanında kalan kayıt haline gelir.`,
    ai: `Yapay zekâ tarafında ${topic} parlak bir demo cümlesiyle değil, veriyle başlayan ve canlı kullanımda ölçülen bir ürün hikâyesiyle anlaşılır. Model, token, embedding, RAG, eval, guardrail veya tool call ancak kaynak, maliyet, güvenlik ve kullanıcı beklentisiyle birlikte okunursa değer üretir.`,
    ...
    interview: `Mülakat hazırlığında ${topic} ezberlenmiş cevapla değil, küçük bir olay hikâyesiyle kalıcı olur. Görüşmeci çoğu zaman yalnız doğru tanımı değil, adayın problemi nasıl parçaladığını, riski nasıl gördüğünü ve bilmediği yeri nasıl dürüstçe sınırladığını dinler.`
  };
  const scene = sceneByMode[category.mode] || sceneByMode.systems;
  const readingAngles = [
    `Bu sayfada zihinsel pusula şu olsun: kavram hangi işi sadeleştiriyor, hangi yeni sorumluluğu doğuruyor ve hata anında hangi izi bırakıyor? ${termLine} Terimler sözlük maddesi gibi ezberlenirse çabuk unutulur; olayın içine yerleşirse yerini bulur.`,
    `Okurken önce görünür faydayı, sonra görünmeyen bedeli izle. ${termLine} Teknolojinin tadı burada çıkar: hızlı çalışan parçanın arkasında bakım, izleme, yetki, maliyet ve geri dönüş planı varsa o tercih gerçekten olgunlaşır.`,
    `Bu başlıkta ezberlenecek kelimeden çok kurulacak bağ önemlidir. ${termLine} Bir terim ancak hangi ekibin eline değdiği, hangi kayıtla doğrulandığı ve hangi kararın riskini azalttığı görüldüğünde gerçek bilgiye dönüşür.`,
    `Kavramı tek cümlelik tanıma sıkıştırmak yerine küçük bir vaka gibi düşün. ${termLine} Kullanıcı ne görür, sistem ne kaydeder, ekip nereden başlar, yöneticiye hangi cümleyle özetlenir? İyi teknik okuryazarlık bu geçişleri rahat yapabilmektir.`
  ];
  const practical = readingAngles[(chapter.number + category.order) % readingAngles.length];
  const fieldStories = [
    `Sahada en öğretici anlar çoğu zaman her şeyin yarım çalıştığı anlardır. Sistem tamamen çökmez ama yavaşlar; hata sürekli değil, aralıklı gelir; tedarikçi kendi tarafının sağlıklı olduğunu söyler; kullanıcı aynı işlemi tekrar deneyince bazen başarılı olur. İşte ${title} böyle anlarda kuru tanımdan çıkar; iyi uzman belirtinin yerini, kapsamını ve son değişikliği sabırla yan yana dizer.`,
    `Bir kurumda küçük görünen teknik başlıkların büyüme biçimi benzerdir: önce birkaç kullanıcı şikâyet eder, sonra destek kaydı açılır, ardından loglar, yetkiler, maliyet raporları veya sözleşme maddeleri masaya gelir. ${title} bu yüzden yalnız teknik bilgi değildir; olayın ne zaman mühendislik, ne zaman operasyon, ne zaman yönetim meselesi olduğunu ayırt etme alıştırmasıdır.`,
    `İyi anlatılmış bir teknoloji konusu, okurun zihninde bir oda açar. O odada yalnız tanım yoktur; uyarı ışığı, karar defteri, sorumlu ekip, ölçülecek metrik ve yanlış kurulumda çıkacak bedel de vardır. ${title} sayfası o odayı doldurmalı: okur bitirdiğinde kavramı hem teknik masada hem yönetici masasının kenarında konuşabilmelidir.`,
    `Gerçek hayatta kavramlar birbirine tek tek değil, düğüm düğüm bağlanır. Bir API cevabı ağdan, kimlikten, veritabanından, logdan ve kullanıcı beklentisinden ayrı düşünülemez; bir ihale maddesi bütçeden, kabulden, bakım sorumluluğundan ve denetim kanıtından koparılamaz. ${title} bu bağları fark ettirdiği ölçüde değerli hale gelir.`
  ];
  const fieldStory = fieldStories[(chapter.number + category.order + 1) % fieldStories.length];
  const modeLens = {
    systems: `Sistem okumasında bu başlığın son cümlesi genellikle ölçümle biter: hangi kaynak tükendi, hangi servis konuşmadı, hangi izin yetmedi, hangi log bize gerçeği söyledi? ${title} okuru bu sakin teşhis diline yaklaştırmalı; çünkü bilgisayarın iç düzenini bilen kişi arıza anında panik yerine sırayla ilerler.`,
    software: `Yazılım tarafında bu konu kod yazmayı yalnız komut dizmekten çıkarır. ${title}, değişikliğin testle güvenceye alınmasını, hatanın anlamlı log bırakmasını, verinin tutarlı kalmasını ve kodun başka biri tarafından okunabilir olmasını hatırlatır. İyi programcı hızlı yazan değil, yarınki bakımcıya yol bırakan kişidir.`,
    ai: `Yapay zekâ başlıklarında romantizm hızlı gelir; bu yüzden ölçü, kaynak ve sınır daha da önemlidir. ${title} konuşulurken veri kalitesi, eval seti, halüsinasyon riski, maliyet, gizlilik ve insan onayı aynı masaya oturmalıdır. Modelin cevabı etkileyiciyse bile soru aynıdır: bunu hangi kanıtla güvenilir sayıyoruz?`,
    security: `Güvenlik okuması burada savunma ürünlerinden önce sorumluluk zincirini görür. ${title}, kimlikten loga, yetkiden olay müdahalesine kadar her adımda kanıt ister. İyi güvenlik dili korku yaymaz; saldırı yüzeyini, etki alanını ve kapatılacak boşluğu netleştirir.`,
    architecture: `Mimari bakışta ${title}, tek parçayı büyütmek yerine parçalar arasındaki trafiği dinletir. Nerede senkron bekliyoruz, nerede kuyruğa bırakıyoruz, hangi veri kimin gerçeği, hangi hata telafi edilebilir? Bu sorular cevaplandığında büyük sistemler karmaşık görünse de okunabilir hale gelir.`,
    project: `Proje ve kamu tarafında ${title}, iyi niyetli teknik cümlenin belgeye, bütçeye, sorumluluğa ve kabule nasıl çevrileceğini gösterir. Bir işin gerçekten teslim edilmiş sayılması için yalnız ekranın açılması yetmez; ölçüt, kayıt, yetki, bakım ve kullanıcı etkisi de masada olmalıdır.`,
    leadership: `Liderlikte ${title}, teknik kararın insana ve kuruma temas eden tarafını açık eder. Ekip kime güvenecek, kullanıcıya ne söylenecek, bütçe hangi gerekçeyle istenecek, kriz sonrası hangi ders kalıcı aksiyona dönecek? Bu soruları sorabilen lider ayrıntıyı küçümsemez; ayrıntıyı karar diline çevirir.`,
    interview: `Mülakat tarafında ${title}, ezber cevaptan çok düşünme ritmini ölçer. Aday önce sade tanım kurar, sonra küçük bir olay anlatır, ardından risk ve kanıt eklerse cevap kendi ayakları üzerinde durur. Bilmediği ayrıntıyı da dürüstçe sınırlandırır; çünkü güven veren uzmanlık biraz da neyi bilmediğini doğru söyleyebilmektir.`
  };
  const textureByMode = {
    systems: [
      `Bu yüzden ${title} okunurken Linux servisleri, dosya izinleri, DNS çözümlemesi, port dinleme durumu, log akışı ve kaynak tüketimi aynı rafın parçaları gibi düşünülmelidir. ` +
        `Bir arıza anında \`systemctl\`, \`journalctl\`, \`curl\`, \`dig\`, \`ss\` veya basit bir log filtresi sihirli komutlar değil, olayı küçülten büyüteçlerdir.`,
      `Burada önemli olan tek aracı ezberlemek değil, aracın sorduğu soruyu anlamaktır. \`top\` kaynak baskısını, \`tail -f\` canlı izi, \`chmod\` yetki sınırını, \`ssh\` uzak makineyle konuşmayı, DNS kaydı ise ismin gerçek adrese nasıl bağlandığını hatırlatır.`,
      `${title} teknik zemini güçlendirirken kullanıcının hissettiği gecikmeyi de gözden kaçırmamalıdır. Ağ geçidi, subnet, sertifika, süreç, bellek ve disk gibi kavramlar ancak hizmet deneyimine bağlandığında kuru donanım bilgisi olmaktan çıkar.`
    ],
    software: [
      `${title} içinde Python, FastAPI, JSON, SQL, PostgreSQL, Redis, Docker, pytest ve OpenAPI gibi adlar geçiyorsa bunları ayrı vitrinler gibi değil, aynı teslimat zincirinin halkaları gibi okumak gerekir. Kod isteği alır, veriyi doğrular, kalıcı kayda dokunur, hata üretirse iz bırakır ve başka ekiplerin okuyacağı bir sözleşme sunar.`,
      `Yazılımda iyi akış, küçük bir fikrin temiz fonksiyona, anlaşılır modüle, test edilebilir davranışa ve geri alınabilir değişikliğe dönüşmesidir. ${title} bu dönüşümün neresinde duruyorsa orada okunabilirlik, veri bütünlüğü ve hata ayıklama alışkanlığı mutlaka masaya gelmelidir.`,
      `Bir kod parçası yalnız çalıştığı günle ölçülmez; altı ay sonra bir başkası onu değiştirebildiğinde gerçekten iyi yazılmış sayılır. Bu nedenle tipler, exception yönetimi, transaction sınırı, log mesajı ve test ismi ${title} içinde görünmez ama belirleyici aktörlerdir.`
    ],
    ai: [
      `${title} için somut zemin veri hattıdır: kaynak belge nereden geliyor, etiket güvenilir mi, embedding hangi anlamı taşıyor, vektör araması doğru parçayı buluyor mu, RAG cevabı kaynağa yaslanıyor mu? PyTorch, TensorFlow, vector database, eval seti ve guardrail ancak bu zincirin içinde anlam kazanır.`,
      `Yapay zekâda iyi anlatım modelin büyüklüğüne takılıp kalmaz; latency, token maliyeti, gizlilik, prompt enjeksiyonu, bias ve insan onayı gibi sessiz maddeleri de sahneye çıkarır. ${title} bu maddeleri görünür kıldığında demo heyecanı ürüne yaklaşır.`,
      `Bir modelin etkileyici cevap vermesi başlangıçtır, bitiş değil. ${title} konuşulurken test verisiyle üretim verisinin ayrımı, halüsinasyon eşiği, geri bildirim döngüsü ve modelin hangi durumda susması gerektiği özellikle anlaşılmalıdır.`
    ],
    security: [
      `${title} güvenlikte tek kontrolle kapanmaz; MFA, IAM, PAM, SIEM, EDR, WAF, şifreleme, yama yönetimi ve olay müdahalesi çoğu zaman aynı hikâyenin farklı cümleleridir. Bir kontrol eksikse saldırgan oradan girer, bir log eksikse ekip ne olduğunu geç anlar.`,
      `Burada asıl mesele korku üretmek değil, saldırı yüzeyini okunur hale getirmektir. ${title}; kimlik, yetki, ağ sınırı, veri sınıfı, alarm kalitesi ve müdahale süresiyle birlikte düşünülürse güvenlik cümlesi olgunlaşır.`,
      `İyi güvenlikçi her şeyi yasaklayan kişi değildir; hangi riskin kabul edilebilir, hangisinin kapatılması zorunlu, hangisinin izlenerek yönetilebilir olduğunu ayırt eder. ${title} bu ayrımı yaptırdığı ölçüde öğretici olur.`
    ],
    architecture: [
      `${title} büyük sistemlerde API gateway, service mesh, message queue, cache, database replica, object storage, observability ve deployment stratejisiyle birlikte düşünülür. Her parça başka bir parçaya söz verir; mimari, bu sözlerin tutulup tutulmadığını izleme sanatıdır.`,
      `Bir mimari kararın olgunluğu genellikle hata anında belli olur. ${title} için senkron çağrı mı asenkron olay mı, tek gerçek veri kaynağı neresi, retry nerede durmalı, idempotency nasıl korunmalı, rollback hangi sınırdan yapılmalı soruları önemlidir.`,
      `Büyük sistem okuryazarlığı, diyagramdaki kutuları saymak değil aralarındaki gerilimi anlamaktır. ${title} performans, tutarlılık, maliyet, ekip sahipliği ve kullanıcı beklentisini aynı resimde topladığında gerçek mimari dersi verir.`
    ],
    project: [
      `${title} proje tarafında RACI, risk kaydı, kapsam yönetimi, kabul kriteri, SLA, bakım sorumluluğu, KVKK etkisi ve tedarikçi sözleşmesiyle aynı dosyada durur. İyi proje metni yalnız ne yapılacağını değil, neyin teslim sayılacağını ve hangi kanıtla kabul edileceğini de tarif eder.`,
      `Kamu ve kurumsal yapılarda teknik karar çoğu zaman resmi yazı, yaklaşık maliyet, teknik şartname, muayene kabul, bakım sözleşmesi ve denetim iziyle yaşar. ${title} bu belgeleri kuru bürokrasi değil, kurum hafızasını ve hesap verebilirliği taşıyan araçlar olarak okutmalıdır.`,
      `Bir projenin kırılma anı genellikle ilk toplantıda değil, teslim yaklaşırken ortaya çıkar: kapsam genişler, veri sahibi değişir, tedarikçi yorum farkı doğar, kullanıcı kabulde başka şey beklediğini söyler. ${title} bu yüzden beklentiyi erken yazıya dökmenin değerini anlatmalıdır.`
    ],
    leadership: [
      `${title} liderlikte SLO, error budget, postmortem, FinOps, kapasite planı, vendor lock-in, yol haritası ve iletişim ritmiyle birlikte okunur. Teknik liderin gücü her ayrıntıyı kendisinin çözmesinden değil, doğru ayrıntıyı doğru kişiye doğru zamanda taşımasından gelir.`,
      `Kriz veya strateji masasında teknoloji dili insan diline çevrilmek zorundadır. ${title}; kullanıcı etkisi, bütçe gerekçesi, risk iştahı, ekip yükü ve kararın geri dönüş maliyetiyle anlatıldığında üst yönetim için okunabilir hale gelir.`,
      `İyi lider teknik konuyu parlatmaz, yerli yerine koyar. ${title} bir ekip kararını etkiliyorsa sahiplik, ölçüm, iletişim ve öğrenme döngüsü de en az teknoloji kadar önem kazanır.`
    ],
    interview: [
      `${title} için güçlü mülakat cevabı tanım, örnek, trade-off, metrik ve sınır beşlisini taşır. Aday Docker, SQL, RAG, IAM veya cache anlatırken bile "nerede kullanırım, neyi ölçerim, yanlış giderse nasıl anlarım" cümlesini kurabiliyorsa cevap ezberden çıkar.`,
      `Üst düzey hazırlıkta konu ne olursa olsun adaydan beklenen şey yalnız bilgi değil, karar kalitesidir. ${title}; sistem tasarımı, kriz okuma, maliyet, güvenlik ve ekip iletişimiyle bağ kurduğunda görüşme odasında daha inandırıcı duyulur.`,
      `Mülakatta her ayrıntıyı bilmek zorunda değilsin; ama ayrıntının nerede aranacağını, hangi varsayımın tehlikeli olduğunu ve hangi kanıtın cevabı güçlendireceğini bilmelisin. ${title} bu kası çalıştıran başlıklardan biri olarak okunmalıdır.`
    ]
  };
  const texturePool = textureByMode[category.mode] || textureByMode.systems;
  const texture = texturePool[(chapter.number + category.order) % texturePool.length];
  const failureByMode = {
    systems: `${title} yanlış kurulduğunda belirti çoğu zaman tek yerde durmaz: uygulama yavaşlar, log büyür, disk dolar, DNS gecikir, sertifika yenilenmez veya yetki modeli sessizce işi kilitler. Bu yüzden sistem bilgisinde küçük işaretleri erken okumak büyük kesintiyi önler.`,
    software: `${title} kötü ele alındığında hata yalnız ekranda patlamaz; veri sessizce bozulabilir, test yanıltıcı geçebilir, API sözleşmesi kırılabilir veya deploy geri alınamaz hale gelebilir. Yazılım ustalığı bu zinciri önceden görme alışkanlığıdır.`,
    ai: `${title} özensiz kurulursa model doğruymuş gibi yanlış konuşabilir, özel veriyi uygunsuz bağlama taşıyabilir veya pahalı bir cevabı gereksiz yere tekrar tekrar üretebilir. Yapay zekâda kalite, cevabın güzelliği kadar hangi durumda güvenilmeyeceğini bilmekle ölçülür.`,
    security: `${title} ihmal edildiğinde olay yalnız teknik açık olmaz; güven kaybı, yasal yük, denetim bulgusu ve operasyon kesintisi de doğurur. Güvenlik dili bu yüzden alarmdan önce hazırlığı, alarmdan sonra kanıtı konuşur.`,
    architecture: `${title} yanlış mimari karara bağlanırsa küçük trafik artışı bile zincirleme gecikmeye, veri tutarsızlığına veya ekipler arası suçlamaya dönüşebilir. İyi mimari, hatanın olacağını kabul edip etkisini sınırlayan tasarımdır.`,
    project: `${title} eksik yönetildiğinde proje çoğu zaman "nerede yanlış anlaştık?" sorusuna saplanır. Kapsam, kabul, sorumluluk ve değişiklik kaydı baştan net değilse en iyi teknik ekip bile teslim gününde savunmasız kalır.`,
    leadership: `${title} liderlikte yanlış tutulursa ekip aynı sorunu farklı isimlerle konuşmaya başlar. Bir taraf performans der, diğer taraf güvenlik, bir başkası maliyet; liderin değeri bu sesleri tek karar zemini üzerinde buluşturmasıdır.`,
    interview: `${title} cevabı yüzeyde kalırsa aday çok şey duymuş ama az şey sindirmiş gibi görünür. Görüşmeci bunu genellikle ikinci soruda anlar: "Peki hata olursa neye bakarsın?" İşte olgunluk o anda başlar.`
  };
  const decisionByMode = {
    systems: `${titleForSentence} için küçük bir karar pusulası kur: önce belirtinin kullanıcının ekranında mı, servis durumunda mı, ağ yolunda mı, yoksa kaynak tüketiminde mi doğduğunu ayır. Sonra ölçümü seç; ping başka şey söyler, log başka şey, metrik başka şey, kullanıcı şikâyeti bambaşka bir şey.`,
    software: `${titleForSentence} için karar pusulası kodun ömrüdür: bugün çalışması, yarın okunması, hata verdiğinde anlaşılması ve veri değiştirdiğinde iz bırakması gerekir. Bu yüzden güçlü cevap yalnız "hangi teknoloji" değil, "hangi sözleşme, hangi test, hangi geri dönüş yolu" sorularını da taşır.`,
    ai: `${titleForSentence} için karar pusulası modelden önce veridir. Kaynağı güvenilir olmayan bilgiyle en iyi model bile kararsızlaşır; iyi tasarım retrieval kalitesini, eval sonuçlarını, kullanıcı geri bildirimini ve güvenlik sınırlarını aynı döngüde izler.`,
    security: `${titleForSentence} için karar pusulası etki alanıdır. Tek kullanıcının hatası mı, ayrıcalıklı hesabın ele geçirilmesi mi, kurum verisinin dışarı sızması mı? Cevap değiştikçe müdahale hızı, iletişim dili ve kanıt ihtiyacı da değişir.`,
    architecture: `${titleForSentence} için karar pusulası bağımlılık haritasıdır. Hangi servis bekliyor, hangisi kuyruğa bırakıyor, hangi veri tekrar üretilemez, hangi hata kullanıcıya gösterilmeden telafi edilebilir? Büyük sistemi okunur yapan bu soruların sırasıdır.`,
    project: `${titleForSentence} için karar pusulası yazılı netliktir. Talep, kapsam, kabul, değişiklik, sorumluluk ve bakım maddeleri baştan görünürse proje yalnız toplantılarla değil, ölçülebilir kararlarla ilerler.`,
    leadership: `${titleForSentence} için karar pusulası etki ve sahipliktir. Kim yoruluyor, kullanıcı ne hissediyor, hangi yatırım riski azaltıyor, hangi karar sonraki krizde işe yarayacak? Liderlik bu soruları teknik ayrıntının içinden çekip çıkarır.`,
    interview: `${titleForSentence} için karar pusulası cevap ritmidir. Önce sade tanım, sonra gerçekçi örnek, sonra trade-off, ardından ölçüm ve sınır. Bu sıra korunursa aday konu değişse bile dağılmaz.`
  };
  const rehearsalLines = [
    `Okur bu sayfadan sonra kendine küçük bir vaka kurabilir: ilk belirti neydi, kim etkilendi, hangi üç kanıta bakıldı, geçici rahatlatma neydi, kalıcı düzeltme kimin sorumluluğuna yazıldı? Böyle düşünmek metni ders notundan çıkarıp saha sezgisine çevirir.`,
    `${titleForSentence} üzerine bir cümle daha eklemek gerekirse, en iyi öğrenme yolu kavramı iki dile çevirmektir: teknik ekibe ayrıntılı, yöneticiye sade. Aynı gerçeği iki dilde anlatabilen kişi konuyu gerçekten sahiplenmeye başlar.`,
    `Bu başlığı çalışırken yalnız doğru cevabı değil, iyi soruyu da ara. "Bunu neden seçiyoruz, neyi feda ediyoruz, nasıl ölçeceğiz, nasıl geri döneceğiz?" soruları ${title} için küçük ama sağlam bir kontrol listesi verir.`,
    `Bir sonraki okuma adımı için bu konuyu zihninde tek bir cümleye indir: hangi sorunu çözüyor, hangi riski açıyor, hangi kanıtla izleniyor? Bu üçlü kurulduğunda teknoloji adı ezber olmaktan çıkar.`
  ];
  const rehearsal = rehearsalLines[(chapter.number + category.order + 3) % rehearsalLines.length];
  const closers = [
    `Bu yüzden ${title} sonunda okurda bırakılacak asıl his, benzer bir olayla karşılaştığında nereden başlayacağını bilme güvenidir.`,
    `Böyle okunduğunda ${title}, teknik ayrıntı olmaktan çıkar ve ekiplerin aynı gerçeği daha az gürültüyle konuşmasını sağlayan ortak dile dönüşür.`,
    `Bu sayfanın hedefi ${title} için daha çok terim saymak değil; doğru soruyu daha erken sormak, doğru kanıta daha hızlı bakmaktır.`,
    `Kavram zihinde bu biçimde yerleşirse ${title}, okuru bir sonraki başlığa yalnız bilgiyle değil, daha iyi çalışan bir teknik sezgiyle taşır.`,
    `Bu nedenle ${title} tek başına akılda kalmakla yetinmez; sonraki bölümlerde karşına çıkacak kararları daha okunur hale getirir.`,
    `Kapanırken ${title} için geride küçük ama işe yarar bir iz kalmalı: kavramı yalnız adıyla değil, kurum içindeki davranışıyla tanımak.`,
    `Okur ${title} başlığından ayrılırken yanında tek bir tanım değil, krizde, toplantıda ve mülakatta kullanabileceği sakin bir düşünme sırası götürmelidir.`,
    `Son cümle ${title} için şudur: teknoloji, ancak insan, süreç, maliyet ve kanıtla aynı masaya oturduğunda gerçekten anlaşılır.`
  ];
  const closer = closers[(chapter.number + category.order + 2) % closers.length];
  return `${scene} ${practical} ${fieldStory} ${modeLens[category.mode] || modeLens.systems} ${texture} ${failureByMode[category.mode] || failureByMode.systems} ${decisionByMode[category.mode] || decisionByMode.systems} ${rehearsal} ${closer}`;
}

function manualArticle(category, chapter) {
  const articleKey = articleContentAliases[chapter.id] || chapter.id;
  const article = articleOverrides[chapter.id] || manualArticles[articleKey];
  if (!article) {
    return [
